...
from mutagen.id3 import ID3, APIC, COMM, TIT2, TPE1, TPE2, TALB, TCOM, TDRC, TRCK, TPOS, TCON, ID3NoHeaderError
    ...
    def apply_tags(self):
        ...
        for file_path in target_files:
            try:
                ...
                # ID3v2.3 Çerçeve Eşleştirmeleri
                frame_map = {
                    'title': (TIT2, 'TIT2'),
                    'artist': (TPE1, 'TPE1'),
                ...
                # 1. Metin etiketlerini yaz / temizle
                for key, (frame_class, frame_id) in frame_map.items():
                    val = tags.get(key, '').strip()
                    full_id3.delall(frame_id)
                    if val:
                        full_id3.add(frame_class(encoding=3, text=val))

                # 2. Yorum (COMM) yaz / temizle
                comment_text = tags.get('comment', '').strip()
                full_id3.delall("COMM")
                if comment_text:
                    full_id3.add(COMM(encoding=3, lang='eng', desc='', text=comment_text))

                # 3. Kapak Resmi (APIC) yaz / temizle
                image_data = data.get('image')
                ...
                # Tümünü tek seferde diske ID3v2.3 formatında yaz
